...
{
    Intersection inter;
    // Task 1.1: Ray-Triangle Intersection
    constexpr float EPSILON = std::numeric_limits<float>::epsilon(); // To avoid errors due to precision

    // Check if the ray is parallel to the triangle
    float det = dotProduct(e1, crossProduct(ray.direction, e2));
    if (std::abs(det) < EPSILON) {
        return inter; // Ray is parallel so no intersection - return default intersection (happened = false)
    }

    // Calculate the barycentric coordinate u
    float invDet = 1.0f / det;
    float u = invDet * dotProduct(ray.origin - v0, crossProduct(ray.direction, e2));
    if (u < -EPSILON || u > 1 + EPSILON) {
        return inter; // Intersection outside triangle - return default intersection (happened = false)
    }

    // Calculate the barycentric coordinate v
    float v = invDet * dotProduct(ray.direction, crossProduct(ray.origin - v0, e1));
    if (v < -EPSILON || u + v > 1 + EPSILON) {
        return inter; // Intersection outside triangle - return default intersection (happened = false)
    }

    // Must be intersection inside triangle, calculate t
    float t = invDet * dotProduct(e2, crossProduct(ray.origin - v0, e1));
    if (t < EPSILON) {
        return inter; // Intersection behind the ray origin - return default intersection (happened = false)
    }

    // Set intersection details
    inter.happened = true;
    inter.coords = ray.origin + ray.direction * t;
    inter.tcoords = Vector2f(u,v); //t0*(1 - u - v) + t1*u + t2*v; 
    inter.normal = normal;
    inter.tnear = t;
    inter.obj = this;
    inter.material = m;
