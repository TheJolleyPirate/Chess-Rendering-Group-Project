...
Eigen::Vector3f normal_fragment_shader(const fragment_shader_payload &payload) {
    ...
    result = payload.normal;
    //convert to range [0, 2] by adding one to all elements
    result = result + Eigen::Vector3f::Ones();
    //convert to range [0, 1] by dividing by 2
    result = result / 2;
    //convert to range [0, 255] by multipliying by 255
    result = result * 255;
    return result;
}
...
Eigen::Vector3f blinn_phong_fragment_shader(const fragment_shader_payload &payload) {
    ...
    Eigen::Vector3f viewDirection = payload.view_pos;
    Eigen::Vector3f normal = payload.normal;
    Eigen::Vector3f result_color = {0, 0, 0};

    Eigen::Vector3f La = ka.cwiseProduct(amb_light_intensity);  // cwiseProduct--dot product
    for (auto &light : lights) {
        Eigen::Vector3f lightDirection = light.position;
        Eigen::Vector3f lightIntensity = light.intensity;
        Eigen::Vector3f LV = lightDirection + viewDirection;
        float absLV = sqrt(pow(LV.x(), 2) + pow(LV.y(), 2) + pow(LV.z(), 2));
        Eigen::Vector3f halfWayVector = LV / absLV;

        Eigen::Vector3f Ld = kd.cwiseProduct(lightIntensity);
        float max = normal.dot(lightDirection);
        if(max > 0){
            Ld *= max;
        }
        else{
        ...
        Ld.x() /= pow(lightDirection.x(), 2);
        Ld.y() /= pow(lightDirection.y(), 2);
        Ld.z() /= pow(lightDirection.z(), 2);

        Eigen::Vector3f Lc = ks.cwiseProduct(lightIntensity);
        max = normal.dot(halfWayVector);
        if(max > 0){
            Lc *= pow(max, p);
        }
        else{
        ...
        Lc.x() /= pow(lightDirection.x(), 2);
        Lc.y() /= pow(lightDirection.y(), 2);
        Lc.z() /= pow(lightDirection.z(), 2);
...
std::tuple<float, float> processUandV(float u, float v){
    float junk;
    u = std::modf(u, &junk);
    v = std::modf(v, &junk);
    if(u < 0){
...
Eigen::Vector3f texture_fragment_shader(const fragment_shader_payload &payload) {
    ...
    if (payload.texture) {
        ...
        auto[u, v] = processUandV(payload.tex_coords.x(), payload.tex_coords.y());
...
void rasterise(rst::rasterizer r, float angle, Vector3f eye_pos, vector<light> lights, vector<Triangle *> TriangleList, rst::Shading shading, string filename, bool commandline, bool shadow){
...
int main(int argc, char const *argv[])
{
    ...
    bool shadow = false;
    string filename = "bob-texture.png";
    //available shaders:
    ...
    objl::Loader Loader;
    string obj_path = "../models/bob-the-duck/";

    // Load .obj File
    bool loadout = Loader.LoadFile("../models/bob-the-duck/bob_tri.obj");
    ...
    auto texture_path = "bob_diffuse.png";
    ...
    if (command_line) {
        rasterise(r, angle, eye_pos, lights, TriangleList, shading, filename, command_line, shadow);
        return 0;
    }
